...
const S = {
  ...
  eliminatedChoices: {}, // {qIdx: Set of eliminated choice indices}
...
function renderKeysList() {
  ...
  cont.innerHTML = S.viewingKeysData.map(sk => {
    ...
    return `<div class="sk-row">
      ...
      ${submitted ? `<button class="btn btn-sm btn-danger" onclick="resetStudent('${sk.id}','${sk.student_name}')" style="padding:3px 8px;font-size:11px;">重置</button>` : '<span style="width:52px;display:inline-block;"></span>'}
    </div>`;
  }).join('');
}
    ...
    if (examId) {
      ...
      if (allNames.length) {
        ...
        if (newNames.length) {
          ...
          needNewKey.forEach((n, i) => { keyMap[n] = newKeys[i]; });
          await api('POST', '/api/student-keys', {
            keys: newNames.map(n => ({ exam_id: examId, student_name: n, student_key: keyMap[n] }))
          });
        }
      }
    } else {
      ...
      if (allNames.length) {
        ...
        needNewKey.forEach((n, i) => { keyMap[n] = newKeys[i]; });
        await api('POST', '/api/student-keys', {
          keys: allNames.map(n => ({ exam_id: examId, student_name: n, student_key: keyMap[n] }))
        });
      }
    }
...
async function studentLogin() {
  ...
  S.role = 'student';
  // 保存学生登录状态到 sessionStorage，刷新后不退出
...
async function renderStudentDashboard(studentName) {
  ...
  // 收集所有科目用于筛选
  const subjects = [...new Set(items.map(i => i.exam.subject).filter(Boolean))].sort();
  const filterBar = subjects.length > 1 ? `
    <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:16px;align-items:center;">
      ...
      ${subjects.map(s => `<button onclick="filterStudentExams('${s}')" id="sf-${s}" class="btn btn-sm" style="font-size:12px;">${s}</button>`).join('')}
    </div>` : '';
  ...
  html += '</div>'; // close student-exam-cards
  cont.innerHTML = html;
  cont._examItems = items; // store for filtering
}
...
function filterStudentExams(subject) {
  ...
  document.querySelectorAll('[id^="sf-"]').forEach(b => {
    b.style.background = 'var(--surface)'; b.style.color = 'var(--text)'; b.style.borderColor = 'var(--border-md)';
  });
  const activeBtn = document.getElementById(subject ? 'sf-' + subject : 'sf-all');
  if (activeBtn) { activeBtn.style.background = 'var(--blue)'; activeBtn.style.color = 'white'; activeBtn.style.borderColor = 'var(--blue)'; }
  const filtered = subject ? items.filter(i => i.exam.subject === subject) : items;
  const sorted = [...filtered].sort((a, b) => (!!a.record) - (!!b.record));
  const cards = document.getElementById('student-exam-cards');
  if (!cards) return;
  cards.innerHTML = sorted.map(({ studentKey: sk, exam, record: rec }) => {
    if (rec) {
      const pct = Math.round(rec.score / rec.total * 100), { fg } = scoreColor(pct);
      ...
      return `<div class="card card-hover mb-2"><div class="flex-between" style="flex-wrap:wrap;gap:12px;"><div style="flex:1;min-width:0;"><div class="flex gap-2 mb-1" style="flex-wrap:wrap;"><span style="font-weight:600;">${exam.name}</span><span class="badge badge-blue">${exam.subject}</span><span class="badge badge-amber">未作答</span>${exam.time_limit_minutes ? `<span class="badge badge-gray">⏱ ${exam.time_limit_minutes} 分钟</span>` : ''}</div>${exam.description ? `<p style="font-size:13px;color:var(--text2);margin-top:4px;">${exam.description}</p>` : ''}</div><button class="btn btn-primary" onclick="startExamWithKey('${sk.id}','${exam.id}')">开始考试</button></div></div>`;
    }
  }).join('');
}
...
async function startExamWithKey(studentKeyId, examId) {
  ...
  S.currentQIdx = 0; S.answers = {}; S.eliminatedChoices = {};
...
function startAntiCheat() {
  ...
  S._acBlur = () => {
    S._acBlurTimer = setTimeout(() => { // 延长到1.5秒，过滤iPad缩放/通知等误触发
      if (!document.getElementById('screen-exam')?.classList.contains('active')) return;
      const overlay = document.getElementById('anticheat-overlay');
  ...
  // 全屏检测：退出全屏算切屏（iPad 双屏防护）
  S._acFullscreen = () => {
    const isFullscreen = !!(document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement);
    if (!isFullscreen && document.getElementById('screen-exam')?.classList.contains('active')) {
      triggerAntiCheat();
      // 弹出提示后尝试重新进入全屏
...
function buildChoiceBtn(l, i, qIdx, sel, elim) {
  ...
  return `<button class="choice-option ${sel && !elim ? 'selected' : ''}" onclick="selectAnswer(${qIdx},${i})"
    style="width:100%;padding:16px 20px;font-size:17px;margin-bottom:10px;display:flex;align-items:center;gap:16px;border-radius:10px;${sel&&!elim?'border-color:var(--blue);background:var(--blue-light);':''}${elimStyle}position:relative;">
    <span style="width:32px;height:32px;border-radius:50%;background:${sel&&!elim?'var(--blue)':'var(--surface2)'};color:${sel&&!elim?'white':'var(--text2)'};font-size:15px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;">${l}</span>
    <span style="font-size:16px;font-weight:${sel&&!elim?'600':'400'};flex:1;">${l}</span>
    <span onclick="toggleEliminate(${qIdx},${i},event)" title="划掉此选项" style="font-size:13px;color:${elim?'var(--red)':'var(--text3)'};padding:4px 6px;border-radius:4px;cursor:pointer;flex-shrink:0;" title="排除此选项">${elim?'↩':'✕'}</span>
  </button>`;
}
...
function selectAnswer(qIdx, choiceIdx) {
  ...
  if (choicesArea) {
    choicesArea.innerHTML = letters.map((l, i) => {
      const sel = S.answers[qIdx] === i;
  ...
  // 更新导航格子
  const navGrid = document.querySelector('#exam-question-area [style*="display:flex;flex-wrap:wrap;gap:5px"]');
  if (navGrid) {
    navGrid.innerHTML = qs.map((_, i) => {
      let bg, color, border;
      if (i === qIdx)                       { bg='var(--blue)';        color='white';        border='var(--blue)'; }
      else if (S.answers[i] !== undefined)  { bg='var(--green-light)'; color='var(--green)'; border='var(--green)'; }
      else                                  { bg='var(--surface)';     color='var(--text3)'; border='var(--border-md)'; }
      return `<button onclick="renderQuestion(${i})" style="width:34px;height:34px;border-radius:6px;font-size:13px;font-weight:700;cursor:pointer;border:2px solid ${border};background:${bg};color:${color};display:inline-flex;align-items:center;justify-content:center;transition:all 0.1s;">${i+1}</button>`;
    }).join('');
  ...
  // 更新已作答计数
  const answered = Object.keys(S.answers).length;
  const countEl = document.querySelector('#exam-question-area span[style*="font-size:13px;color:var(--text2)"]');
...
async function submitExam() {
  ...
  document.getElementById('exam-timer').style.display = 'none';
  // 用服务器返回的正确答案覆盖题目数据（仅用于结果页显示）
  if (result.correct_answers) {
    qs.forEach((q, i) => { q.correct_answer = result.correct_answers[i]; });
  }
  showResults(result.score, result.total, qs);
}
